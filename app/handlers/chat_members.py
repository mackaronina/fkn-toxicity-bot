...
async def update_new_member(event: ChatMemberUpdated, bot: Bot) -> None:
    ...
    await bot.send_animation(event.chat.id, FSInputFile(f'{BASE_DIR}/app/static/images/join.mp4'), caption=name,
...
async def update_kick_member(event: ChatMemberUpdated, bot: Bot) -> None:
    name = html.escape(event.new_chat_member.user.full_name)
    await bot.send_animation(event.chat.id, FSInputFile(f'{BASE_DIR}/app/static/images/kick.mp4'), caption=name,
                             show_caption_above_media=True)
...
async def update_leave_member(event: ChatMemberUpdated, bot: Bot) -> None:
    name = html.escape(event.new_chat_member.user.full_name)
    await bot.send_animation(event.chat.id, FSInputFile(f'{BASE_DIR}/app/static/images/leave.mp4'), caption=name,
                             show_caption_above_media=True)
